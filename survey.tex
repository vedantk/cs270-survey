...
restricted to $\im(L) = \Span(\one)^{\bot}$, we have:
\begin{align*}
...
and $\tilde{L}$ are very close. In this section, we show that sparsifiers
which keep the quadratic forms of $\Pi$ and $\tilde{\Pi}$ close achieve
this.
